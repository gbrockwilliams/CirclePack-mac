 ...
 * tilings via associated circle packings. Tilings may be 
 * stored in files, the packing being generated on being 
 * read, may be generated by, e.g., 'pave', or may be
 * built in hierarchical fashion from subdivision rules. 
 * Much of this code is devoted to this latter scenario.
 * 
 * The basic combinatorics of a tiling are stored in a 
 * 'TileData' structure, which is a pattern of tiles, 
 * each defined by a closed, counterclockwise chain of 
 * vertices, starting with the "principal" vertex, which
 * is 'vert[0]'.
 * 
 * A major concern involves a family of tilings generated 
 * by some finite subdivision rule; see class 
 * 'SubdivisionRules'. Calling for some depth of 
 * subdivision generates a hierarchy of tilings. This is 
 * reflected in the element 'gradedTileData' in 
 * 'canonicalPack.tileData'. If 'packData.tileData' 
 * has the deepest level tiling, then it is just a 
 * pointer to 'canonical.tileData'. However, 
 * 'packData.tileData' may represent some lesser depth in
 * the hierarchy, in which case its 'myTiles' are bare 
 * bones clones at one level in the hierarchy and do not 
 * contain full hierarchy information. 
 * 
 * A global tiling at a given stratum is defined 
 * recursively: the subtilings of the top tiles are found 
 * and then pasted together, but those subtilings are 
 * pasted together from their subtilings, etc. So one 
 * recurses on pasting together tiles until one reaches 
 * down to the desired level. It is assumed, in order 
 * to do the pastings, that at each stratum, neighboring 
 * tiles have edges with the same number of vertices, so 
 * they can be pasted. This compatibility is part of the 
 * art in defining subdivision rules.
 * 
 * This extender builds the tilings at the full requested 
 * depth and creates the associated circle packing as it 
 * goes along. It keeps track of the tilings at all 
 * intermediate stages in 'gradedTileData' as well and 
 * it stores all the vertices defining tiles at every 
 * stage in the tile's 'augmented' vert list.
 *
 * There are four levels of combinatorial complexity 
 * for packings one can associate with tilings:
 * 
 * (1). The minimal has a vertex for each tile corner 
 * and a barycenter vertex for each tile. This is mode=1
 * 
 * (2). Next is the barycentric subdivision, which adds 
 * a barycenter vert to each edge: this is a standard 
 * topological barycentric subdivision of the tiling 
 * as a cell complex. This is mode=2. See 'baryize'.
 * 
 * (3). (default) Third is a further barycentric refinement 
 * of the barycentric subdivision from (2), mode=3. Each 
 * face is further subdivided as a hex flower. This 
 * parallels the structure introduced with "dessins" 
 * and is realized in 'canonicalPack' for continual 
 * reference. Problems in tracing edges (e.g., there 
 * are three shortest complex edges between two tile 
 * corners) is taken care of with the 'augmented' 
 * vertex lists.
 * 
 * (4) Finally, there may be various refinements of 
 * any of these. Its tiles are displayed using 
 * 'canonicalPack' information; the link between these 
 * can be corrupted --- e.g., by swapping indices.
 * 
 * CAUTION: 'canonicalPack' has lots of info: the 
 * tiles, dual tiles, quad tiles, white/grey tiles, etc. 
 * This has to be generated from the input 'TileData' of 
 * tiles alone. This extra info is NOT saved in packing 
 * files --- only the original tiles are saved. Thus, to
 * use this extra info one must always reconstruct it using 
 * this pack extender. 
 * 
 * When constructing subdivision tilings to some depth d, 
 * intermediate tilings stored in 'tileData.gradedTileData'; 
 * see 'set_TD'. Tile 'vert's and 'augVert's are all in terms 
 * of circles of the parent packing. In particular, 'mode' 
 * should be set initially, it can't really be reset 
 * without regenerating the packing from scratch. 
 * 
 * We create and save 'canonicalPack'; if there is no 
 * tiling, then a packing with TileData can be loaded 
 * or a tiling generated using 'pave'. '|ct| export -c {p}' 
 * will export 'canonicalPack' to pack 'p', 
 * '|ct| get_canon' will replace parent 'packData' with 
 * a copy of 'canonicalPack'.
 * 
 * The aim of this extender is to create/modify tilings, 
 * generate new packings which can replace 'packData' or 
 * be copied or saved, and generate associated dual, 
 * quad, and grey/white tilings.
 * 
 * CAUTION: 'canonicalPack' is fixed after it is formed, 
 * but 'packData' or its copy of 'TileData' might be 
 * changed. The idea is to use 'canonicalPack', its 
 * 'TileData', and 'TileData.gradedTileData' as a base 
 * to spawn and modify other tilings. 'put_canon' will 
...
public class ConformalTiling extends PackExtender {
	...
	public int mode; // almost always use mode 3
	  // mode 1: simple, each n-tile just gets barycenter
      // mode 2: barycentrically subdivide the n edges also
	  // mode 3: barycentrically subdivide the 2n faces (typical)
	
	// To save recomputation in recursive construction of 
		// tilings at various depths, on reading a rules 
		// file, we save the 'TileData' for the subdivision 
		// of each tile type in a vector. 'topTileData(j)' 
		// has subdivision TileData for tile type j 
		// (recall types start at 4).
	Vector<TileData> topTileData;
	// As packings are built 'depthPackings(j)' stores 
	//    vector of packings for tile type j. Thus vec(0) 
	//    (created on reading in a rules file) is a packing 
	//    (barycentrically subdivided, mode 3) for a single 
	//    tile of type j; vec(d), if it exists for d>0, is 
	//    a packing for a tile of type j subdivided to level d.
	//    During 'build2Depth' recursion, copy from 
	//    depthPackings(j)(d) if it exists, else create/save 
	//    for later use. 
...
//	static boolean buildDeBug=true; 
	...
	public ConformalTiling(PackData p) {
		...
		extensionAbbrev="CT";
		toolTip="'ConfomalTiling' creates/manipulates tilings, "
				+ "particularly subdivision tilings. Tiling data is "
				+ "stored with maximal packing 'canonicalPack' and "
				+ "can be copied to/from and manipulated in parent packing.";
		registerXType();
		subdivRules=null;
	...
	public int cmdParser(String cmd,Vector<Vector<String>> flagSegs) {
		...
		if (cmd.startsWith("subti")) {
			...
			if (depthPackings!=null)
				...
				if (tpe<0)
					Oops("This tiling has only "+
							subdivRules.type2Rule.size()+" types");
					...
					while (ink.hasNext()) {
						...
						if (t>=1 && t<currTD.tileCount) { // legal index?
							...
							if (atd!=null) { // has children?
								for (int cld=1;cld<=atd.tileCount;cld++) { // add child's index
									nextLink.add(atd.myTiles[cld].tileIndex); 
								}
							}
						}
		...
		// ================ store_eucl ================
		// preparing to draw traditional euclidean tilings.
		//   Saving the tile vertex locations requires the rule 
		//   file to have had optional eucl position data 
		//   (perhaps 3D); tile marks are set to depth.
		//   Caution: center and radius data is modified.
		//   TODO: alternative is to write the info to a 
		//   file in "eucl3D" command, which is under 
		//   development (1/2018).
		if (cmd.startsWith("store_eucl")) {
			...
			if (canonicalPack.tileData.gradedTileData==null || 
				...
				throw new ParserException(
						"for 'store_eucl', there must be just one top level tile");
...
//	      	initBase[1]=new Complex(1.0);
		...
		// ================ feedback ===========
			...
			try {
				...
				if (d1<0 || d1> depth)
					throw new ParserException(
							"given depth, "+d1+" is improper or not available");
				...
				while (items.size()>=2) {
					fromtoPairs.add(new EdgeSimple(Integer.parseInt(items.remove(0)),
							Integer.parseInt(items.remove(0))));
				}
...
//			}
		...
		// ============ subtile vertlist =============
			...
			if (canonicalPack.tileData==null || 
					canonicalPack.tileData.gradedTileData==null) {
				errorMsg("No 'canonicalPack' or no subdivision "
						+ "tilingsrules in place");
				return 0;
			}
			
			if (flagSegs==null || flagSegs.size()==0) {
				errorMsg("usage: stvl t -n, 't' tile number, "
						+ "'n' number of generations above it");
				return 0;
			}
			...
			if (theFile!=null) {
				...
				try {
					...
					while (rls.hasNext()) {
						// TODO: rules <-> types currently in 1-to-1 corres.
						TileRule trule=rls.next(); 
						TileData tdata=SubdivisionRules.
								getRulesTD(trule,subdivRules.type2Size,mode);
			...
			while (fs.hasNext() && (items=fs.next())!=null && items.size()>0) {
				...
				if (mode==4) {
					...
					while (tlk.hasNext()) {
						...
						TileRule tr=subdivRules.tileRules.
								get(subdivRules.type2Rule.findW(tile.tileType));
					...
					// want greywhite
						...
						try {
							...
							throw new DataException(
									"Problem accessing 'wgTiles' for white/grey tiles");
						}
					}
					else { // not greywhite
...
//			packData.tileData=canonicalPack.tileData.copyBareBones();
				...
				if (StringUtil.isFlag(str)) {
						...
						// format "-d5" for depth 5.
					{
						...
						if (depth>canonicalPack.tileData.gradedTileData.size()-1)
							depth=canonicalPack.tileData.gradedTileData.size()-1;
						TileData holdTD=canonicalPack.tileData.
								gradedTileData.get(depth).copyBareBones();
	 ...
	 */
	public PackData buildMode3(TileData td) {
		...
		newPack.tileData.builtMode=3;
		newPack.tileData.setParent(newPack);
		newPack.set_aim_default();
		cpCommand(newPack,"layout -F");
		if (cpCommand(newPack,"max_pack")<=0) 
			errorMsg("failed in max packing newPack");
		return newPack;
	}
	...
	public static PackData trilize(TileData td) {
		...
		// create new minimal packing for the tiling
	 ...
	 * 
	 * One problem is convention for boundary edges: dual's 
	 * are 'half'ed: boundary tiles are closed up with boundary 
	 * edges, so the dual to a boundary edge ends at a "corner" 
	 * of the dual tile, the vertex associated with the dual 
	 * tile (a corner in the origin) is a corner. NOTE: 
	 * building a packing from the dual will add barycenters
	 * to these half tile, hence change the combinatorics; 
	 * so dual of the dual is not quite the original.
	 * @param td TileData
...
//		return (TileData.tiles2canonical(newPD.tileData));
	 ...
	 * packing with a fully realized tiling (i.e., mode 3).
	 * @return PackData, null on error
	 */
	public PackData baryize() {
		TileData td=null;
		if (canonicalPack==null || 
				(td=canonicalPack.tileData)==null || 
				td.tileCount<=0) 
			return null;
		...
		TileData newTD=new TileData(tcount,1);
		...
		for (int t=1;t<=td.tileCount;t++) {
			...
			for (int j=0;j<tile.vertCount;j++) {
				...
				int m=tile.augVert[4*j+2];
				int c=tile.augVert[(4*j+4)%tile.augVertCount];

				// white triangle
				Tile nt=newTD.myTiles[++tick]=new Tile(3);
				nt.tileIndex=tick;
				nt.vert[0]=bary;
				nt.vert[1]=a;
				nt.vert[2]=m;
				...
				// grey triangle
				nt=newTD.myTiles[++tick]=new Tile(3);
				nt.tileIndex=tick;
				nt.vert[0]=bary;
				nt.vert[1]=m;
				nt.vert[2]=c;
		...
		// create packing from new tiling.
		PackData newPD = buildMode3(newTD);    // tiles2FullBary(newTD);
	...
	public PackData deltize(TileData td) {
		...
		// create packing from new tiling.
		newPD = buildMode3(newPD.tileData);
		if (newPD==null || newPD.tileData==null)
			return null;
		newPD.tileData.gradedTileData=new Vector<TileData>();
	 ...
	 * @return int
	 */
	public int setEuclRecurs(PackData p,SubdivisionRules sRules,Tile toptile,
			Complex []base,int depth) {
		...
		// first, locate your own vertices 
		//   (just vertices, not augmented vertices)
		for (int j=0;j<topRule.stdCorners.length;j++) {
			p.setCenter(toptile.vert[j],
					new Complex(topRule.stdCorners[j].times(basedir).add(origin)));
		...
		if (depth > 0) {
			for (int n = 1; n <= topRule.childCount; n++) {
				...
				int rslt = setEuclRecurs(p,sRules, toptile.myTileData.myTiles[n], 
						subtileBase, depth - 1);
				if (rslt <= 0)
					return 0;
				count += rslt;
...
//		boolean debug=false;
		...
		outData.subRules=tData.subRules;
	 ...
	 */
	public static PackData copyDepthPacking(Vector<Vector<PackData>> depthPDs,
			int tiletype,int depth) {
	...
	public void initCmdStruct() {
		...
		cmdStruct.add(new CmdStruct("set_mode","[mode]",null,
				"Mode 1 = simple (this can fail); 2 = with edge barycenters;"
		...
		cmdStruct.add(new CmdStruct("disp",
				"-[[g]TDQ m] {j..} -[E -m{x}] {t..}",null,
				"display specified tiles: T = tiles, D = dual tiles, "
				+ "Q = quad tiles, m = mark distinquished corners, "
				+ " E = augmented tile edges marked with 'x'; usual "
				+ "display options for color, thickness, fill. "
				+ "List appropriate tiles, vertices, or edge barycenters. "
				+ "[g] preceding flag means display associated grey/white "
				+ "subtiles instead."));
		cmdStruct.add(new CmdStruct("set_tflowers",null,null,
				"Set the 'tileFlower' data, telling each tile "
				+ "who its neighbors are"));
		cmdStruct.add(new CmdStruct("mark",null,null,
				"Set 'mark' for all vertices: 1 = tile barycenter; "+
				"2 = dual tile barycenter; 3 = quad tile barycenter; else 0"));
		cmdStruct.add(new CmdStruct("read_canon","{filename}",null,
				"Read a tiling file to replace 'canonicalPack'"));
		cmdStruct.add(new CmdStruct("get_canon",null,null,
				"convert the parent packing to a clone of 'canonicalPack'"));
		cmdStruct.add(new CmdStruct("put_canon",null,null,
				"Use tiledata in parent to create a new 'canonicalPack'. "
				+ "The parent is unchanged."));
		cmdStruct.add(new CmdStruct("export","-[mbc] {p}",null,
				"export a tile packing of level n (m=minimal, "
				+ "c=canonicalPack, b=barycentric) to packing p"));
		cmdStruct.add(new CmdStruct("godual","x",null,
				"replace parent packing by the packing for the dual "
				+ "tiling, x to also replace 'canonicalPack'. "
				+ "NOTE: original packing is lost."));
		cmdStruct.add(new CmdStruct("split3",null,null,
				"replace 'packData' with minimal packing but with "
				+ "each tile edge split into three, mark 1 2 3 4 "
				+ "(baryVert, corner, two splits)"));
		cmdStruct.add(new CmdStruct("subdivide","{type}",null,
				"subdivide: type [shbqptd] star, "
				+ "hex, bary, quad, pent, twist, delta"));
		cmdStruct.add(new CmdStruct("Vlist","-[TDQ][v] {j..}",null,
				"fill 'Vlist' using specified tile "
				+ "indices 'j': list baryVerts, or with optional 'v', "
				+ "list tile vertices. 'T', 'D', 'Q' "
				+ "for tiles, dual tiles, or quad tiles"));
		cmdStruct.add(new CmdStruct("load_rules","[[-s] filename]",null,
				"Load a Bill Floyd style *.r subdivision rule file"));
		cmdStruct.add(new CmdStruct("pave","v",null,
				"This tries to create a tiling for "
				+ "'packData' made of its disjoint interior flowers, "
				+ "starting with v."));
		cmdStruct.add(new CmdStruct("stvl","{t} -{n}",null,
				"set vlist as boundary of tile 'n' levels up "
				+ "from tile 't'"));
		cmdStruct.add(new CmdStruct("build_sub","-d {x}",null,
				"Apply subdivision process to the current tiling, "
				+ "depth 'x'. Resulting 'canonicalPack' is copied "
				+ "to active packing."));
		cmdStruct.add(new CmdStruct("subtile","{t} -d {x} -m {j}",null,
				"Create a tiling with single tile of "
				+ "type 't', then subdivide to depth 'x' with "
				+ "'mode' j (1, 2, or 3); original packing is lost"));
		cmdStruct.add(new CmdStruct("?angles","{t}",null,
				"List vertices and interior angle sums for tile 't'"));
		cmdStruct.add(new CmdStruct("feedback","-d {d} {{f1 t1}..{fn tn}}",
				null,"Feedback bdry angle data from tiles fj of a core "
						+ "at depth d > 0 to corresponding "
						+ "tiles tj at depth 0; core should be simply "
						+ "connected. Set parent bdry aims to Pi, then "
						+ "use feedback from the tiles at depth d to bdry "
						+ "vertices of parent. Then ready to repack. "
						+ "Suitable for eucl case only."));
		cmdStruct.add(new CmdStruct("store_eucl",null,null,
				"Store eucl centers for tile vertices if the geometric "
				+ "data is provided (optional, in *.r rules file)"));
		cmdStruct.add(new CmdStruct("write_eucl",null,null,
				"Send euclidean tiling to postscript file if the "
				+ "geometric data is provided (optional, in *.r rules file)"));
		cmdStruct.add(new CmdStruct("draw_eucl",null,null,
				"Draw euclidean tiling if the geometric data is "
				+ "provided (optional, in *.r rules file)"));
		cmdStruct.add(new CmdStruct("set_tlist","[-P{d} {t..}]",null,
				"fill 'tlist' or 'Tlist' with tiles whose parents "
				+ "at depth 'd' are in the list {t..} (default, "
				+ "one level up)"));
	}
